...
def extract_cx_fallback(response: str, valid_ids: Optional[List[str]] = None) -> str:
    """Last-resort extraction when \\boxed{} is absent.

    Searches for C\\d+ option IDs in the response, preferring the tail (where
    the final answer is typically stated) and pipe-separated groups.
    Never hurts score vs blank since wrong-single gives IoU=0 same as blank.
    """
    if not response:
        return ""
    # 1. Look for explicit pipe-separated group: C5|C8|C11
    pipe = re.search(r'\bC(\d+(?:\|C\d+)+)\b', response)
    if not pipe:
        pipe = re.search(r'(C\d+(?:\s*[|,]\s*C\d+)+)', response)
    if pipe:
        raw_ids = re.findall(r'C\d+', pipe.group(0))
        if valid_ids:
            raw_ids = [i for i in raw_ids if i in valid_ids]
        if raw_ids:
            return "|".join(sorted(set(raw_ids), key=lambda x: int(re.search(r'\d+', x).group())))

    # 2. Extract last C\d+ from the last 400 chars of the response
    tail = response[-400:]
    matches = re.findall(r'\bC(\d+)\b', tail)
    if not matches:
        matches = re.findall(r'\bC(\d+)\b', response)
    if not matches:
        return ""
    candidates = [f"C{m}" for m in matches]
    if valid_ids:
